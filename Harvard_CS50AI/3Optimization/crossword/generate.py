...
class CrosswordCreator():
    ...
    def order_domain_values(self, var: Variable, assignment: dict) -> list:
        ...
        neighbors -= set(assignment.keys())
        overlaps = dict(self.crossword.overlaps)
        used = set(assignment.values())
        mine = self.domains[var]

        def sort_val(val: str):
            cnt = 0
            for it in neighbors:
                idx, idy = overlaps[var, it]
                for val2 in self.domains[it]:
                    if val[idx] != val2[idy]:
                        cnt += 1
            return cnt

        return sorted(list(mine-used), key=sort_val)
    ...
    def select_unassigned_variable(self, assignment: dict) -> Variable:
        ...
        """
        def sort_var(variable: Variable):
            choices = self.domains[variable]
            neighbors = self.crossword.neighbors(variable)
            return (len(choices), -len(neighbors))

        varis = self.crossword.variables.copy() - set(assignment)
        if len(varis) != 0:
            return sorted(list(varis), key=sort_var)[0]
        return None
